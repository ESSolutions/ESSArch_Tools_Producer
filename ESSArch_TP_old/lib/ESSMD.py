...
#Imports
import pytz, datetime, string, csv, os, uuid, ESSPGM, time, urllib, logging, shutil
...
def PREMIS2METS(SIPpath,ObjectIdentifierValue,AgentIdentifierValue,altRecordID_dict={'POLICYID':10,'PROJECTNAME':'xyz123'},METSfile=None):
    ...
    ###########################################################
    # get policy info
    logger.info('Start to create METS for: %s', ObjectIdentifierValue)
    ...
    if errno:
        logger.error('Failed to access Local DB, error: ' + str(why))
        ok = 0
    if ok:
        ###########################################################
        # set variables
...
#    if metatype in [1,2,3]:
    if ok:
        ...
        if errno == 0:
            logger.info('Succeeded to get object_list from premis for information package: %s', ObjectIdentifierValue)
        else:
    ...
    if ok:
        ...
        for object in object_list:
            ...
            if firstPremisObjectFlag:
                if object[0] == ObjectIdentifierValue:
                    logger.info('First premis object match information package: %s', ObjectIdentifierValue)
                    firstPremisObjectFlag = 0
                    continue
                else:
        ...
        if ok:
            try:
                ...
                logger.warning('ObjectIdentifierValue: %s is not a valid UUID, why: %s , setting OBJID prefix to RAID: in METS' % (ObjectIdentifierValue, str(why)))
    ...
    if not errno:
        for f in res:
            ...
            if os.path.exists(src_f):
                ...
                logger.warning('missing file: %s' % src_f)
        shutil.move(os.path.join(SIProotpath,'TIFFEdit.RES'),SIPcontentpath)
    else:
        logger.warning('missing RESfile: %s' % os.path.join(SIProotpath,'TIFFEdit.RES'))
