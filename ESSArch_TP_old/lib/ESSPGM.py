...
__date__ = "$Date: 2013-02-21 12:18:55 +0100 (Thu, 21 Feb 2013) $"
__author__ = "$Author: henrik $"
import re
__version__ = '%s.%s' % (__majorversion__,re.sub('[\D]', '',__revision__))

import hashlib, subprocess, ESSMD, logging, time, datetime, os, stat, re, tarfile, urllib, uuid, csv, smtplib, email.charset, pytz, mimetypes, sys
...
class PROC:
    def action(self,action,Name=None,id=None):
        ...
        ############### Stop #################
        if action == 'Stop':
            ESSDB.DB().action('ESSProc','UPD',('Status','2','Run','0'),('id',id))
        ############### Kill #################
        if action == 'Kill':
...
class Check:
    ...
    "Get filetree"
    ###############################################
    def GetFiletree2(self,path,checksum=None):
                ...
                if os.access(path, os.R_OK) and os.access(path, os.W_OK) and os.access(path, os.X_OK):
                    for self.f in os.listdir(path):
                        ...
                        if os.access(self.path, os.R_OK):
                            ...
                            if stat.S_ISREG(self.mode[0]):                   # It's a file
                                if checksum:
                                    self.f_checksum, errno, why = Check().checksum(self.path,checksum)
                                    if errno:
                                        return self.file_list, errno, why
                                    self.f_mimetype = mimetypes.guess_type(self.path,True)[0]
                                    if self.f_mimetype is None:
                                        return self.file_list, 2, 'Filetype %s is not supported' % self.f
                                else:
                                    self.f_checksum = None
                                    self.f_mimetype = None
                                self.file_list.append([Check().str2unicode(self.f), os.stat(self.path), self.f_checksum, self.f_mimetype])
                            elif stat.S_ISDIR(self.mode[0]):                 # It's a directory
                                self.dir_file_list, errno, why = Check().GetFiletree2(self.path,checksum)
                                if not errno:
                                    for self.df in self.dir_file_list:
                                        self.file_list.append([self.f + '/' + self.df[0], self.df[1], self.df[2], self.df[3]])
                                else:
                                    return self.file_list, errno, why
                        else:
                            self.exitstatus = 12
    ...
    def rescheck(self, resfile, ressize, tarfile):
        #logger.info('Start to check tifsize and restifsize for: ' + tarfile)
    ...
    def checksum(self,fname,ChecksumAlgorithm = 1):
        #logger.info('Start to create %s checksum for: %s' % (ChecksumAlgorithm, fname))
...
#                    logfile.close()
    ...
    def Create_IP_package(self,ObjectIdentifierValue,ObjectPath,Package_ObjectPath,METS_ObjectPath=None,aic_object=None,TimeZone=timezone.get_default_timezone_name(),checksumtype_default='MD5'):   
        ...
        if self.ok:
            ...
            self.res_info, self.res_files, self.res_struct, errno, why = ESSMD.getMETSFileList(FILENAME=self.Cmets_objpath)
            if errno == 0:
                logger.info('Succeeded to get object_list from METS for information package: %s', self.ObjectIdentifierValue)
            else:
                self.event_info = 'Problem to get object_list from METS for information package: %s, errno: %s, detail: %s' % (self.ObjectIdentifierValue,str(errno),str(why))
        ...
        if self.ok:
            ...
            try:
                logger.info('Create IP Package: ' + self.p_objpath)
                ...
                if os.access(self.filepath_iso,os.R_OK):
                    if int(os.stat(self.filepath_iso)[6]) == self.object_size:
                        try:
                            ...
                            logger.info('Add: ' + self.a_filepath + ' to IP Package: ' + self.p_obj)
                        except tarfile.TarError:
    ...
    ##############################################
    def AIPextract(self,storageMediumID=None,ObjectIdentifierValue=None,complete=None,delete=1,prefix=None,target=None,unpack=0):
        ...
        self.numOK = 0
        if complete:
            logger.info('Start complete verify storageMediumID: ' + str(self.storageMediumID))
        elif ObjectIdentifierValue:
            logger.info('Start verify ObjectIdentifierValue: ' + str(ObjectIdentifierValue) + ' from storageMediumID: ' + str(self.storageMediumID))
        else: 
            logger.info('Start quick verify storageMediumID: ' + str(self.storageMediumID))
        ...
        #logger.info('self.sql: ' + str(self.sql))
        ...
        logger.debug('self.NumOfRows: %s, len(self.checks): %s, self.checks: %s',str(self.NumOfRows),str(len(self.checks)),str(self.checks))

        for self.check in self.checks:
            logger.debug('self.numOK: %s',str(self.numOK))
            ...
            self.ObjectPath = os.path.join(self.verifydir,self.ObjectIdentifierValue + '.tar')
            ...
            while 1:
                ...
                if len(self.res_ioqueue):
                    if int(self.res_ioqueue[0][0]) == 20:
                        ...
                        logger.info('Checksum verify OK for storageMediumID: ' + str(self.storageMediumID) + ' ObjectIdentifierValue: ' + str(self.ObjectIdentifierValue))
                        break
                    elif int(self.res_ioqueue[0][0]) > 20:
                        self.VerifyChecksum = 1
                        logger.error('Checksum verify failed!! for storageMediumID: ' + str(self.storageMediumID) + ' ObjectIdentifierValue: ' + str(self.ObjectIdentifierValue))
                        break
                time.sleep(1)
            if unpack:
                logger.info('Start to unpack %s', self.ObjectIdentifierValue)
                if self.storageMediumFormat in range(100,102):
                    self.res,errno,why = Check().AIPunpack(self.ObjectIdentifierValue,self.verifydir,0,self.VerifyChecksum)
                else:
                    self.res,errno,why = Check().AIPunpack(self.ObjectIdentifierValue,self.verifydir,1,self.VerifyChecksum)
                if errno:
                    self.numOK -= 1
            ...
            if delete == 1:
                ...
                if self.storageMediumFormat in range(100,102):
                    logger.info('Try to remove ObjectPath: ' + self.ObjectPath)
                else:
                    logger.info('Try to remove ObjectPath: ' + self.ObjectPath + ' and ' + self.PMetaObjectPath)
                try:
                    ...
                    if self.storageMediumFormat in range(100,102):
                        logger.error('Problem to remove ObjectPath: ' + self.ObjectPath)
                    else:
                        logger.error('Problem to remove ObjectPath: ' + self.ObjectPath + ' and ' + self.PMetaObjectPath)
                else:
                    if self.storageMediumFormat in range(100,102):
                        logger.info('Success to removeObjectPath: ' + self.ObjectPath)
                    else:
                        logger.info('Success to removeObjectPath: ' + self.ObjectPath + ' and ' + self.PMetaObjectPath)
            elif delete == 2:
                ...
                if self.storageMediumFormat in range(100,102):
                    ...
                    logger.info('Try to remove ObjectPath: ' + self.ObjectPath + ' and ' + self.CMetaObjectPath + ' and ' + self.PMetaObjectPath)
                try:
                    os.remove(self.ObjectPath)
                    if not self.storageMediumFormat in range(100,102):
                    ...
                    if self.storageMediumFormat in range(100,102):
                        ...
                        logger.error('Problem to remove ObjectPath: ' + self.ObjectPath + ' and ' + self.CMetaObjectPath + ' and ' + self.PMetaObjectPath)
                else:
                    if self.storageMediumFormat in range(100,102):
                        logger.info('Success to removeObjectPath: ' + self.ObjectPath)
                    else:
                        logger.info('Success to removeObjectPath: ' + self.ObjectPath + ' and ' + self.CMetaObjectPath + ' and ' + self.PMetaObjectPath)
        logger.debug('last self.numOK: %s',str(self.numOK))
    ...
    def AIPunpack(self,ObjectIdentifierValue,target,mets = 1,VerifyChecksum = 1):
        ...
        if self.ok and self.mets_flag:
            ...
            if errno == 0:
                logger.info('Success to get object_list from premis for information package: %s', self.ObjectIdentifierValue)
                #logger.debug('Meta_filepath: %s , object_list: %s', self.Meta_filepath,str(self.object_list))
            else:
        ...
        if self.ok and not self.mets_flag:
            ...
            if errno == 0:
                logger.info('Success to get object_list from RES for information package: %s', self.ObjectIdentifierValue)
            else:
        ...
        if self.ok and self.mets_flag:
            ...
            logger.info('Format validate object: ' + self.ObjectIdentifierValue)
        ...
        if self.ok and not self.mets_flag:
            ...
            logger.info('Format validate object (RES): ' + self.ObjectIdentifierValue)
            self.startTime = datetime.timedelta(seconds=time.localtime()[5],minutes=time.localtime()[4],hours=time.localtime()[3])
            self.ObjectNumItems = 0
            self.ObjectSize = 0
            for self.object in self.object_list:
...
class Robot:
    ...
    ###############################################
    def Inventory(self):
        logger.info('Start to inventory robot')
        ...
        if self.inv.returncode:
            logger.error('Faild to inventory robot, error: %s',str(self.invout))
            return 1
        else:
            logger.debug('Robot inventory output: %s',str(self.invout))
            return 0

    "Update localDB whith volserDB from robot"
    ###############################################
    def GetVolserDB(self,CentralDB=1,set_storageMediumLocation='',set_storageMediumLocationStatus=''):
        logger.info('Start to GetVolserDB')
        ...
        if self.RobotStat.returncode:
            logger.error('Faild to get status from robot, error: %s',str(self.RobotStatout))
            return 1
        else:
            logger.debug('Robot status output: %s',str(self.RobotStatout))
        self.line = ''
                    ...
                    if not errno: 
                        if not len(res) == int(self.info[7]):
                            ...
                            for slot_id in range(1,int(self.info[7])+1):
                                res,errno,why = ESSDB.DB().action('robot','INS',('status','None',
                                                                                 't_id','',
                                                                                 'drive_id','99',
                                                                                 'slot_id',slot_id))
                    ...
                    if not re.search('EXPORT',self.line):
                        ...
                        if self.s_elements[4] == 'Full':
                            ...
                            if len(self.s_elements) >= 7:
                                ...
                                # Slot is occupied
                                if CentralDB:
                                    logger.info('Try to sync local DB from central DB for mediaID: %s',self.s_elements[6][:6])
                                    ...
                                    if errno == 1:
                                        ...
                                        logger.info('Check if mediaID: %s exist in local DB',self.s_elements[6][:6])
                                        res,errno,why = ESSDB.DB().action(self.StorageMediumTable,'GET3',('storageMediumID',),
                                                                                                         ('storageMediumID',self.s_elements[6][:6]))
                                        if errno:
                                            logger.error('Failed to access local DB: %s',str(why))
                                            return 10
                                        elif len(res) > 1:
                                            logger.error('To many storagemedias found in local "storageMedium" DB for %s',self.s_elements[6][:6])
                                            self.TapeExistFlag = 1
                                        elif len(res) == 1:
                                            logger.info('Found storageMedia %s in local DB', self.s_elements[6][:6])
                                            self.TapeExistFlag = 1
                                    elif errno == 2 or errno == 3:
                                        logger.info('No archive objects to sync for mediaID: %s from central DB, exit code: %s',self.s_elements[6][:6],str(errno))
                                        self.TapeExistFlag = 1
                                    elif errno > 3:
                                        logger.error('Failed to sync mediaID: %s from central DB, errno: %s',self.s_elements[6][:6],str(errno))
                                        return 30
                                    else:
                                        ######################################################
                                    ...
                                    if errno:
                                        ...
                                        self.TapeExistFlag = 1
                                if Debug: 
                                    logger.debug('self.s_elements[6][:6]: ' + str(self.s_elements[6][:6]))
                                    logger.debug('self.TapeExistFlag: ' + str(self.TapeExistFlag))
                                if self.TapeExistFlag:                        #Check if t_id exist in archtape
                                    ...
                                    if errno:
                                        logger.error('Failed to update location for MediumID: %s , error: %s',self.s_elements[6][:6],str(why))
                                else:
                                    res,errno,why = ESSDB.DB().action('robot','UPD',('status','Ready',
                                                                                    ...
                                                                                    ('slot_id',self.s_elements[3]))
                                if errno:
                                    logger.error('Failed to update local "robot" DB: %s, %s error: %s', str(self.s_elements[3]), self.s_elements[6][:6], str(why))
                                    return 11
                            else:
                                #######################################################
                                # Slot is empty
                                res,errno,why = ESSDB.DB().action('robot','UPD',('status','None',
                                                                                ...
                                                                                ('slot_id',self.s_elements[3]))
                                if errno:
                                    logger.error('Failed to update local "robot" DB: %s, none, error: %s', str(self.s_elements[3]), str(why))
                                    return 11
                            ...
                            if self.mounted_slot_id:
                                res,errno,why = ESSDB.DB().action('robot','UPD',('status','Mounted',
                                                                                ...
                                                                                ('slot_id',self.s_elements[3]))
                                if errno:
                                    logger.error('Failed to update local "robot" DB: %s, %s error: %s', str(self.s_elements[3]), self.mounted_slot_id[0][1], str(why))
                                    return 11
                            else:
    ...
    def MTPosition(self, tapedev, t_num):
        ...
        if errno:
            ...
            return 'Fail'
        logger.info('Start to position to tapefile: ' + str(t_num) + ' current position is: ' + str(self.real_current_t_num))
        if int(t_num) == 0:
            ...
            logger.info('Start to position with: bsfm: ' + str(self.newt_num))
            ...
            if self.newt_num > 0:
                logger.info('Start to position with: fsf: ' + str(self.newt_num))
                ...
                self.newt_num = 1
                logger.info('Start to position to beginining of tape file with: bsfm: ' + str(self.newt_num))
                self.cmd = subprocess.Popen(["mt","-f",str(tapedev),"bsfm",str(self.newt_num)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        if self.cmd.returncode == 0:
            ...
            if errno:
                ...
                logger.info('Success to position to tapefile: ' + str(t_num) + ' cmdout: ' + str(self.mtout))
                return 'OK'
        else:
            logger.error('Problem to position to tapefile: ' + str(t_num) + ' cmdout: ' + str(self.mtout))
            return 'Fail'
    ...
    def MountWritePos2(self,t_type,t_block,t_format,t_prefix,t_location,full_t_id='',work_uuid=''):
        ...
        if len(self.full_t_id) == 6:
            ...
            if errno:
                ...
                if errno: logger.error('Failed to update Local DB: ' + str(self.full_t_id) + ' error: ' + str(why))
                if errno == 0 and ExtDBupdate:
                    ext_res,ext_errno,ext_why = ESSMSSQL.DB().action(self.StorageMediumTable,'UPD',('storageMediumStatus','100',
                                                                                                    'linkingAgentIdentifierValue',AgentIdentifierValue),
                                                                                                   ('storageMediumID',self.full_t_id))
                    if ext_errno: logger.error('Failed to update External DB: ' + str(self.full_t_id) + ' error: ' + str(ext_why))
                    else:
        ...
        if self.t_id:
            ...
            if self.robotdrive:
                ...
                while 1:
                    ...
                    if self.robotdrive:
                        ...
                        if Debug: logger.info('Wait for mounting of: ' + str(self.t_id))
                    time.sleep(2)
            ...
            if self.t_id: 
                self.t_id=self.t_id[0][0]
                logger.info('No writetape found, start to mount new tape: ' + str(self.t_id))
                ##########################
                ...
                res,errno,why = ESSDB.DB().action(self.StorageMediumTable,'INS',('storageMedium',t_type,
                                                                                 ...
                                                                                 'LocalDBdatetime',self.timestamp_utc.replace(tzinfo=None)))
                if errno: logger.error('Failed to insert to Local DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(why))
                if errno == 0 and ExtDBupdate:
                    ...
                    if ext_errno: logger.error('Failed to insert to External DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(ext_why))
                    else:
                        res,errno,why = ESSDB.DB().action(self.StorageMediumTable,'UPD',('ExtDBdatetime',self.timestamp_utc.replace(tzinfo=None)),
                                                                                        ('storageMediumID',self.t_id))
                        if errno: logger.error('Failed to update Local DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(why))
                ############################
                # Mounting new tape
                ESSDB.DB().action(self.RobotReqTable,'INS',('job_prio','1','status','Pending','req_type','Mount','t_id',self.t_id,'work_uuid',self.work_uuid))
                while 1:
                        ...
                        try:     #Open tapedevice
                            ...
                            if errno == 0 and ExtDBupdate:
                                ext_res,ext_errno,ext_why = ESSMSSQL.DB().action(self.StorageMediumTable,'UPD',('storageMediumStatus','100',
                                                                                                                'linkingAgentIdentifierValue',AgentIdentifierValue),
                                                                                                               ('storageMediumID',self.t_id))
                                if ext_errno: logger.error('Failed to update External DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(ext_why))
                                else:
                                    ...
                                    if errno: logger.error('Failed to update Local DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(why))
                            logger.error(self.t_id + ' failed to open tapedevice, Message: ' + str(why))
                        else:
                            logger.info(self.t_id + ' succeed to open tapedevice')
                            try:      #Add tapelabel to tape
                                logger.info(self.t_id + ' start to add label tape')
                                ...
                                if errno == 0 and ExtDBupdate:
                                    ...
                                    if ext_errno: logger.error('Failed to update External DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(ext_why))
                                        ...
                                        res,errno,why = ESSDB.DB().action(self.StorageMediumTable,'UPD',('ExtDBdatetime',self.timestamp_utc.replace(tzinfo=None)),('storageMediumID',self.t_id))
                                        if errno: logger.error('Failed to update Local DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(why))
                                logger.error(self.t_id + ' failed to write tapelabel, Message: ' + str(why))
                            else:
                                logger.info(self.t_id + ' succeed to label new media')
                            try:      #Close tapedevice
                                ...
                                if errno == 0 and ExtDBupdate:
                                    ...
                                    if ext_errno: logger.error('Failed to update External DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(ext_why))
                                        ...
                                        res,errno,why = ESSDB.DB().action(self.StorageMediumTable,'UPD',('ExtDBdatetime',self.timestamp_utc.replace(tzinfo=None)),('storageMediumID',self.t_id))
                                        if errno: logger.error('Failed to update Local DB: ' + str(self.t_id) + ' (ESSPGM) error: ' + str(why))
                                logger.error(self.t_id + ' failed to close tapedevice, Message: ' + str(why))
                            else:
                                logger.info(self.t_id + ' close tapedevice succeed')
                ...
                logger.error('No empty tapes are avilable in robot with tape prefix: ' + str(t_prefix))
        ...
        self.db_t_pos = ESSDB.DB().action(self.StorageTable,'GETlast',('contentLocation','contentLocationValue'),('storageMediumID','=','"'+self.t_id+'"'))
        if len(self.db_t_pos) == 0 and self.new_tape_flag == 1: 
            self.t_pos = 1
        elif len(self.db_t_pos):
            self.t_pos = int(self.db_t_pos[0][1]) + 1
        else:
            logger.error(str(self.t_id) + ' missing tape contentlocation in DB (StorageTable)')
            return 1, self.t_id, self.tapedev, 0
        logger.info(self.t_id + ' start to position to writeposition ' + str(self.t_pos))
        if Robot().MTPosition(self.tapedev, self.t_pos) == 'OK':
            # Tape is in write position
            logger.info(str(self.t_id) + ' is in writeposition ' + str(self.t_pos))
            return 0, self.t_id, self.tapedev, self.t_pos
        else:
            # Problem to position tape
            logger.error(str(self.t_id) + ' has problem to position to writeposition ' + str(self.t_pos))
...
class svardb:
    ...
    def ArchObjUpd(self,a_obj,a_num,t_id):
        ...
        if re.search('Lyckades',self.data):
            logger.info('Success to update PRJ_db with bacthinfo: ' + str(a_obj))
        else:
            logger.error('Problem to update PRJ_db with bacthinfo: ' + str(a_obj))
    ...
    def ArchObjVer(self,a_obj):
        ...
        self.conn.close()
        if re.search('Lyckades',self.data):
            logger.info('Success to update PRJ_db with verifyinfo: ' + str(a_obj))
        else:
            logger.error('Problem to update PRJ_db with verifyinfo: ' + str(a_obj))
...
class ExtPrjDB:
    ...
    def taped(self,object,tifnum,tifsum,date,time,t_id1,t_id2,t_pos1,t_pos2):
        ...
        #self.urlupdate = "http://212.181.19.10/web/svarfolder/admin/skanning/update_database.asp?%s"
        self.urlupdate = self.url + '?%s'
        logger.info('Try to update ExtPrJDB with taped info: ' + str(object) + ' self.urlupdate: ' + str(self.urlupdate) + ' self.params: ' + str(self.params))
        try:
            self.conn = urllib.urlopen(self.urlupdate % self.params)
        except (IOError), (errno,why):
            logger.error('Problem to connect to URL: ' + str(self.url) + '?' + str(self.params) + ' Error: ' + str(why) + ' ' + str(errno))
        else:
            self.data = self.conn.read()
            self.conn.close()
            if re.search('OK ',self.data):
                logger.info('Success to update ExtPrJDB with taped info: ' + str(object) + ' cmdout: ' + str(self.data))
            else:
                logger.error('Problem to update ExtPrJDB with taped info: ' + str(object) + ' cmdout: ' + str(self.data))
...
class Events:
    ...
    def create(self,eventType,eventDetail,eventApplication,eventVersion,eventOutcome,eventOutcomeDetailNote,UpdateMode,ObjectIdentifierValue=None,storageMediumID=None,eventDateTime=None,linkingAgentIdentifierValue=None,storageMediumLocation=None,storageMediumDestination=None,RelatedEventIdentifierValue=None):
        ...
        if eventDateTime:
            if type(eventDateTime) == datetime.datetime:
                if eventDateTime.tzinfo == None:
                    logger.error('eventDateTime missing tzinfo(ex.+01:00): ' + str(eventDateTime))
                    return 3
                elif eventDateTime.tzinfo == pytz.utc:
                    self.timestamp_utc = eventDateTime.replace(microsecond=0)
                    self.timestamp_dst = self.timestamp_utc.astimezone(self.tz)
                else:
                    logger.error('eventDateTime tzinfo is not UTC(ex.+00:00): ' + str(eventDateTime))
                    return 4
            else:
                logger.error('eventDateTime is not type datetime.datetime: %s, %s' % (type(eventDateTime), str(eventDateTime)))
                return 5
        else:
            self.timestamp_utc = datetime.datetime.utcnow().replace(microsecond=0,tzinfo=pytz.utc)
            self.timestamp_dst = self.timestamp_utc.astimezone(self.tz)
        ...
        if not errno and self.updateDB:
            ...
            logger.error('eventType missing for code: ' + str(eventType))
            self.updateDB=(0,0)
            return 2
        ##########################################################
        ...
        if self.updateDB[0] and ObjectIdentifierValue: 
            ...
            if errno==5:
                logger.warning('Problem to update local eventDB for eventType: ' + str(eventType) + ', object: ' + Check().unicode2isostr(ObjectIdentifierValue) + ', error: ' + str(errno) + ', why: ' + Check().unicode2isostr(why))
                return 5
            elif errno:
                logger.error('Problem to update local eventDB for eventType: ' + str(eventType) + ', object: ' + Check().unicode2isostr(ObjectIdentifierValue) + ', error: ' + str(errno) + ', why: ' + Check().unicode2isostr(why)) 
                return 10
        ...
        #Update local eventDB for storagemedium object
            ...
            if errno:
                logger.error('Problem to update local eventDB for eventType: ' + str(eventType) + ', storageMediumID: ' + str(storageMediumID) + ', error: ' + str(errno) + ', why: ' + Check().unicode2isostr(why))
                return 11
        ...
        if UpdateMode == 2 and self.updateDB[1] and ObjectIdentifierValue:
            ...
            res,errno,why=ESSMSSQL.DB().action('eventIdentifier','INS',('eventIdentifierValue',self.uuid,
                                                                        ...
                                                                        'linkingObjectIdentifierValue',ObjectIdentifierValue))
            if errno:
                logger.error('Problem to update AIS eventDB for eventType: ' + str(eventType) + ', object: ' + Check().unicode2isostr(ObjectIdentifierValue) + ', error: ' + str(errno) + ', why: ' + Check().unicode2isostr(why))
                return 20
        ...
        #Update externalDB (AIS) for storagemedium object
            ...
            if errno:
                logger.error('Problem to update AIS eventDB for eventType: ' + str(eventType) + ', storageMediumID: ' + str(storageMediumID) + ', error: ' + str(errno) + ', why: ' + Check().unicode2isostr(why))
                return 21
        return 0
...
#    @classmethod
    ...
    def subtar(self,packagefile,blksize,workdir,SIPfile,Metafile,AICObjectFILE=None):
        ...
        self.tarout = self.cmd.communicate()
        if self.cmd.returncode == 0:
            if Debug: logger.info('Succeed to write files to tape, stdout: ' + str(self.tarout[0]) + ' stderr: ' + str(self.tarout[0]) + ' exitcode: ' + str(self.cmd.returncode))
        else:
            if Debug: logger.error('Problem to write files to tape, stdout: ' + str(self.tarout[0]) + ' stderr: ' + str(self.tarout[0]) + ' exitcode: ' + str(self.cmd.returncode))
